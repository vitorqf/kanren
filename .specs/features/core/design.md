...
*Pro:* Round-trip logic isolated and pure = easy tests + easy Go teaching. Single I/O owner = one place conflicts/validation live. No npm, no build step.
*Con:* SSE + fsnotify is new territory (mitigated: tiny surface, P1 story WEB-03).

**B — Fat card package (card does its own file I/O).**
*Pro:* fewer packages.
*Con:* I/O scattered, hard to test purely, validation duplicated between CLI/web. Rejected.

**C — SQLite index cache alongside files.**
Files stay source of truth; a SQLite cache speeds queries.
*Pro:* fast queries at scale.
*Con:* two sources of truth = the exact divergence we promised to avoid; cache invalidation on `git pull`. Over-engineered for MVP (boards are dozens–hundreds of cards, linear scan is fine). Rejected for MVP; revisit only if perf demands.
